...
```
```{r}
#Risso dolphin density map

Gg_dolphin <- dolphin_clean %>%
  filter(Species == "Gg")

Gg_density_map <- ggplot() +
  geom_sf(data = GOM_shp, color = "darkgrey", fill = "lightgrey") +
  geom_point(Gg_dolphin, mapping = aes(x = Longitude, y = Latitude), color = "blue", size = 0.5) +
  geom_density_2d(Gg_dolphin, mapping = aes(x = Longitude, y = Latitude)) +
   xlim(-68.7, -66.4) +
  ylim(43.6, 44.6) +
  theme.dgm.map() +
  labs(x = " \nLongitude", y = "Latitude\n ") +
...
ggsave(plot = Gg_density_map, filename = here("Figures_Tables", "Gg_denisty_map.png"), width = 7.5, height = 5.5, 
    units = "in", pointsize = 15, dpi = 220)
```
```{r}
